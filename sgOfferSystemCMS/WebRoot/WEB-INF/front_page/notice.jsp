				...
				<c:forEach items="${pagination.list }" var="item">
					...
					<div class="thumbnail product-item">
						...
						<div class="caption">
							...
							时间:${item.date}<br/>
							<c:if test="${item.text==null and item.remark==1}">
							状态:<span id="status" class="label label-success">
									未反馈
									</span>
							<br/>
							</p><a onclick="" class="btn btn-success btn-block" role="button" disabled=true>等待反馈</a>
							</div>
							</c:if>
							<c:if test="${item.text eq '无货' and item.remark==1}">
							状态:<span id="status" class="label label-success">
									无货
									</span>
							<br/>
							</p><a onclick="" class="btn btn-success btn-block" role="button" disabled=true>暂时无货</a>
							</div>
							</c:if>
							<c:if test="${item.remark==3}">
							状态:<span id="status" class="label label-success">
									${item.text}
									</span>
							<br/>
							</p>
							<a onclick="" class="btn btn-success btn-block" role="button" disabled=true>已购买</a>
							</div>
							</c:if>
							
							<c:if test="${item.text ne '无货' and item.text ne null and item.remark ne 3}">
							状态:<span id="status" class="label label-success">
									${item.text}
									</span>
									<br>
									</p>
								<a onclick="buy(${item.id})" id="buyBtn" class="btn btn-success btn-block" role="button">购买</a>
							</div>
							</c:if>
