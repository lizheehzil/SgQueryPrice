...
public class MyWebSocketHandler implements WebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session)
		...
		if (userSocketSessionMap.get(uid) == null) {
			...
			for (Message message : list) {
				...
				sendMessageToUser(message.getTo(), new TextMessage(
	...
	public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
			...
			if(msg.getTo()==3213123){
				if(msg.getType()==3){
					System.out.println("员工发起购买");
					messageDao.buy(msg.getId());
				}else if(msg.getType()==1){
					System.out.println("员工发起预订");
					messageDao.insertUnread(msg);
				}
			}else if(msg.getFrom()==3213123){
					System.out.println("门店反馈");
					messageDao.feedback(msg);
			}
