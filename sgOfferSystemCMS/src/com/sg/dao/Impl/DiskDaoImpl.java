...
public class DiskDaoImpl implements DiskDao {
	...
	public int isExist(String pno){
		String GETCOUNT = "select count(id) from disk where pno='"+pno+"'";
	...
	public int getCount() {
		String GETCOUNT = "select count(id) from disk";
	...
	public Pagination getAll(int pageNo, String term, String term2, String term3) {
		...
		StringBuilder sb=new StringBuilder("select * from disk where is_display =1 ");
		
		
		if(!term.equals("")) {
			Integer d=Integer.parseInt(term)-100;
			Integer u=Integer.parseInt(term)+100;
			sb.append("and capacity > ").append(d+" and capacity < "+u+" ");
			}
		if(!term2.equals("")) sb.append("and brandId ='").append(term2+"' ");
		if(!term3.equals("")) sb.append("and interface_type_id like '%").append(term3+"%' ");
	...
	public int importDisk(List<Disk> list) {
		String ADD = "insert into disk (id,pno,brandId,category_id,model,capacity,"
				+ "interface_type_id,cost_price,mini_price,inner_price,media_price,"
		...
		for (Disk disk : list) {
			if(isExist(disk.getPno())!=0){
				deleteDisk(disk.getPno());
			}
				Object[] args = new Object[] { disk.getId(), disk.getPno(),
						disk.getBrandId(), disk.getCategory_id(), disk.getModel(),
						disk.getCapacity(), disk.getInterface_type_id(),
						disk.getCost_price(),disk.getMini_price(), disk.getInner_price(),
						disk.getMedia_price(), disk.getJd_price(),disk.getVip_price(),
						disk.getCreate_time(),disk.getUpdate_time(), disk.getRemark(), 
						disk.getIs_display() };
				...
				Object[] args1=new Object[]{disk.getId(),disk.getPno(),disk.getCategory_id(),"Disk"};
	...
	public Pagination searchdisk(int pageNo, String term, String term4) {
		...
		if(!term.equals("")) {
			sb.append("and model like '%").append(term+"%' ");
			}
	...
	public Disk getItem(String pno) {
		String sql="SELECT * FROM disk WHERE pno='"+pno+"'";	
		List<Disk> list = new ArrayList<Disk>();
		jdbcTemplate.query(sql,
				new RowCallbackHandler() {
	...
	public int saveDisk(Disk u) {
		...
		String save="update disk set model=?,interface_type_id=?,capacity=?,cost_price=?,"
				+ "mini_price=?,inner_price=?,media_price=?,jd_price=?,remark=?,is_display=?,"
				+ "update_time=? where pno=?";
		int s=jdbcTemplate.update(save,new Object[]{u.getModel(),u.getInterface_type_id(),
				u.getCapacity(),u.getCost_price(),u.getMini_price(),u.getInner_price(),u.getMedia_price(),
	...
	public void deleteDisk(String pno) {
		String delete="delete from disk where pno=?";
	...
	public List<Brand> getBrand() {
		String sql="select id from category where name='硬盘'";
		int category_id;
		try {
