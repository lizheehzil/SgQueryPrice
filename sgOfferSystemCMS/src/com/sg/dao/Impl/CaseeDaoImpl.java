...
@Repository
public class CaseeDaoImpl implements CaseeDao {
	...
	public int isExist(String pno){
		String GETCOUNT = "select count(id) from casee where pno='"+pno+"'";
	...
	public int getCount() {
		String GETCOUNT = "select count(id) from casee";
	...
	public Pagination getAll(int pageNo, String term, String term2) {
		...
		StringBuilder sb=new StringBuilder("select * from casee where is_display =1 ");
		
		if(!term.equals("")) sb.append("and brandId ='").append(term+"' ");
	...
	public int importCasee(List<Casee> list) {
		String ADD = "insert into casee (id,pno,brandId,category_id,model,"
		...
		for (Casee casee : list) {
			if(isExist(casee.getPno())!=0){
				deleteCasee(casee.getPno());
			}
				Object[] args = new Object[] { casee.getId(), casee.getPno(),
						casee.getBrandId(), casee.getCategory_id(), casee.getModel(),
						casee.getCost_price(),casee.getMini_price(), casee.getInner_price(),
						casee.getMedia_price(), casee.getJd_price(),casee.getVip_price(),
						casee.getCreate_time(),casee.getUpdate_time(), casee.getRemark(), 
						casee.getIs_display() };
				...
				Object[] args1=new Object[]{casee.getId(),casee.getPno(),casee.getCategory_id(),"Casee"};
	...
	public Casee getcasee(Integer id) {
		String sql="SELECT * FROM casee WHERE id="+id;	
	...
	public Casee getItem(String pno) {
		String sql="SELECT * FROM casee WHERE pno='"+pno+"'";	
		List<Casee> list = new ArrayList<Casee>();
		jdbcTemplate.query(sql,new RowCallbackHandler() {
					@Override
					public void processRow(ResultSet rs) throws SQLException {
	...
	public int saveCasee(Casee u) {
		...
		String save="update casee set model=?,cost_price=?,"
				+ "mini_price=?,inner_price=?,media_price=?,jd_price=?,remark=?,is_display=?,"
				+ "update_time=? where pno=?";
		int s=jdbcTemplate.update(save,new Object[]{u.getModel(),
	...
	public void deleteCasee(String pno) {
		String delete="delete from casee where pno=?";
	...
	public List<Brand> getBrand() {
		String sql="select id from category where name='机箱'";
		int category_id;
		try {
