...
public class MemoriesController {
	...
	@RequestMapping("getAllMemories.shtml")
	public String querynotpads(@RequestParam(value = "pageNo", required = false) Integer pageNo,
		...
		Pagination pagination = memories_barDao.getAll(pageNo,term,term2,term3);
		String url = "getAllMemories.shtml";
		terms.append(t1).append(t2).append(t3);
		pagination.pageView(url, terms.toString());
		model.addAttribute("pagination", pagination);
		return "memories";
	}
	@RequestMapping("detail_memories.shtml")
	public String todetail(String brand,String category, Integer id, Model model){
		Memories_bar arg=memories_barDao.getmemories(id);
		model.addAttribute("brand",brand);
		model.addAttribute("category", category);
		model.addAttribute("arg",arg);
		return "detail/detail_memories";
	}
}
